...
def main():
    ...
    while syöte != "lopeta":
        ...
        if syöte == "lisää":
            ...
            nimi = input("Syötä 1. operandin nimi: ")
            if nimi in sanakirja:
                nimi2 = input("Syötä 2. operandin nimi: ")

                if nimi2 in sanakirja:
                    oso = sanakirja[nimi].gib_oso() * sanakirja[nimi2].gib_oso()
                    oso = int(oso)
                    nimit = sanakirja[nimi].gib_nimit() * sanakirja[nimi2].gib_nimit()
                    nimit = int(nimit)
                    luku = Murtoluku( oso,nimit )

                    print(sanakirja[nimi],"*",sanakirja[nimi2],"=",luku)
                    luku.sievennä()
                    print("eli sievennettynä",luku )
                else:
                    print( "Nimeä {} ei ole olemassa".format(nimi2) )
            else:
                print("Nimeä {} ei ole olemassa".format(nimi) )
        elif syöte == "listaa":
            avaimet = sanakirja.keys()
            avaimet = sorted( avaimet )
