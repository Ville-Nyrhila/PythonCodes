...
def jaa_murtoluvut(mluku1 = Murtoluku, mluku2 = Murtoluku):
    ...
    print("eli sievennettynä " + mluku3.gib_form() )
#
def summaa_murtoluvut(mluku1 = Murtoluku, mluku2 = Murtoluku):
    nimit1 = mluku1.gib_nimit()
    nimit2 = mluku2.gib_nimit()

    mluku4 = Murtoluku( mluku1.gib_oso()*nimit2, mluku1.gib_nimit()*nimit2 )
    mluku5 = Murtoluku( mluku2.gib_oso()*nimit1, mluku2.gib_nimit()*nimit1 )

    oso = mluku4.gib_oso() + mluku5.gib_oso()
    nimit = mluku4.gib_nimit()# * mluku2.gib_oso()
    mluku3 = Murtoluku(oso, nimit)
    ...
    print("eli sievennettynä " + mluku3.gib_form() )
#
def vähennä_murtoluvut(mluku1 = Murtoluku, mluku2 = Murtoluku):
    nimit1 = mluku1.gib_nimit()
    nimit2 = mluku2.gib_nimit()
...
def main():
    ...
    try:
        ...
        operaatio = input("Syötä laskuoperaattori (+, -, * tai /) tai vertailu (< jne): ")
        ...
        if operaatio == "*":
            ...
            summaa_murtoluvut(mluku1, mluku2)

        elif operaatio == "-":
            vähennä_murtoluvut(mluku1, mluku2)

        elif operaatio == "<":
            print("{} < {} = {}".format(mluku1.gib_form(), mluku2.gib_form(), mluku1<mluku2) )
